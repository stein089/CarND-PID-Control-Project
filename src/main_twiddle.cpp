...
// Best error 14.9227
//Kp=0.813491  Ki=0.00215393  Kd=3.24184


const int RUNS = 100;
const int N_PER_RUN = 500;


double best_error = -1;
double p [3] = {0.813491, 0.00215393, 3.24184}; 
...
int main()
{
  ...
  throttle_pid.Init(0.05, 0.001, 0);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        //  std::cout << "Total Error " << steer_pid.TotalError() << std::endl;
          ...
          if (count > N_PER_RUN || cte > 5)
          {
            if (cte>5)  // off road
              steer_pid.total_err = 1000;
            ...
            if (flag_first_try)   // First try
            {
              ...
              {
                std::cout << "Decrease " <<  p[twiddle_index%3] << std::endl;
                p[twiddle_index%3] = p[twiddle_index%3] - 2 * dp[twiddle_index%3];
                std::cout << "After Decrease " <<  p[twiddle_index%3] << std::endl;
                steer_pid.Init(p[0], p[1], p[2]);
